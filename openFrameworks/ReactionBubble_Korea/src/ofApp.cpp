...
void ofApp::processSensorVals(){
    ...
    if (WEARABLES){
        ...
        // A1 - ADD, C3 - DELETE
        // ALT BEAM BREAKS:
        // D4 - INCREMENT, E5 - DECREMENT
        
        // WEARABLE BEAM A1
        if (sensorVals[0] == 2000){ //use code instead of 65000
            if (!beamBroken[0]){
                addDrawVid();
                beamBroken[0] = true;
                ...
                ofLogNotice("Beam: ") << "A1 triggered new video" << endl;
            } else {
                ofLogVerbose("Beam: ") << "A1 beam already triggered" << endl;
            }
            
        } else {
            beamBroken[0] = false;
            ofLogVerbose("Beam: ") << "A1 beam reset" << endl;
        }
        
        // WEARABLE BEAM D4
        if (sensorVals[3] == 5000){ //use code instead of 65000
            if (!beamBroken[3]){
                stopDrawVid();
                beamBroken[3] = true;
                ...
                ofLogNotice("Beam: ") << "D4 removed oldest video" << endl;
            } else {
                ofLogVerbose("Beam: ") << "D4 beam already triggered" << endl;
            }
            
        } else {
            beamBroken[3] = false;
            ofLogVerbose("Beam: ") << "D4 beam reset" << endl;
        }
        ...
        if (sensorVals[4] == 65000){
            if (!beamBroken[4]){
                ...
                ofLogNotice("Beam break: ") << "E5 triggered new video" << endl;
            } else {
...
void ofApp::keyPressed(int key){
    ...
    switch(key){
        ...
        case '-':
            ...
            // use white as near value - not sure what this does
        case 'w':
        {
            bool nearWhite = !(kinect.isDepthNearValueWhite());
            kinect.enableDepthNearValueWhite(nearWhite);
            string isNearWhite = "not ";
            if (nearWhite)
                isNearWhite = "";
            ofLogNotice("Kinect") << "near white is " << isNearWhite << "enabled";
        }
            break;
